...
DB-graph Logseq plugin SDK quirks (property API — including the `hide?` deletion/display gotchas — theming, dev workflow, debugging) and the local HTTP API for probing a running graph are collected in [`logseq-sdk-notes.md`](./dev-notes/logseq-sdk-notes.md). Check there first when an `@logseq/libs` call behaves unexpectedly.

## Architecture

> New to this codebase? Start with [`dev-notes/architecture.md`](./dev-notes/architecture.md) (high-level framing, tech stack, the cross-process mental model) and [`dev-notes/module-map.md`](./dev-notes/module-map.md) (where everything lives). The sections below are the detailed reference those two point into.
...
The single configuration surface: `Reference Manager: Settings` opens `SetupContainer` → `SetupApp` (`src/features/setup/`). The native settings panel is reduced to a launcher + connection-status line — its keys stay declared (rows hidden via injected CSS, `HIDDEN_KEYS` in `settings.ts`) only so Logseq seeds defaults on a fresh install. The left-nav has three groups: **General** (Schema, Authors — the shared schema + creator formatting both sources inherit), **Zotero** (Connection, Import formats, Attachments, Annotations, Tag rules), **Web references**. Simple controls autosave (`updateSettings`); the heavy ops — Apply schema, schema delete, Tag rules' Save, Set up web tag — are explicit, and `SetupApp` lands on the first incomplete gating section (`GATING = ['connect', 'schema']`).

The full reference lives in [`settings.md`](./dev-notes/settings.md): every key (type / default / who reads it), each section's controls, the save/dirty-snapshot model, the hidden-keys CSS trick, and how to add a setting. The annotation config and the web-clipper contract get their own callouts there too.
...
6. Attachments + annotations get inserted under a wrapping block whose name is the `attachmentsBlockName` setting (default `Attachments`). Per attachment, the emitted block depends on its Zotero `linkMode`:
   ...
   - **Everything else** (non-PDF `linked_file`, `imported_file` including PDFs there, `linked_url`) — emitted as a plain markdown link. `openAttachmentInline`'s `!` prefix toggles embed vs. external open. URL form per mode follows the rules in [`dev-notes/logseq-file-links.md`](./dev-notes/logseq-file-links.md) — bare absolute path with literal characters for `linked_file`, Zotero's enclosure URL for `imported_file`, the web URL for `linked_url`.
...
Before importing, the user clicks **Apply schema** (`Reference Manager: Settings` → Schema), which runs `services/set-logseqdb-schema.ts`: it creates the base tag (`zotTag`, default `Reference`), upserts each preset property with the right type (`node` / `date` / `url` / `default`) and associates it with the tag, then — if `webTag` is set — makes the web class `extends` the base (`ensureWebTagExtendsBase`, `services/set-web-schema.ts`) so web clips inherit the same idents without per-property re-association. Presets are `Essentials` / `Full` / `Custom` (defined in `src/constants.ts`). The Schema **Danger zone** (`services/delete-zotero-schema.ts`) removes every plugin-created property by the `ZOTERO_PROP` namespace (never the user's own), leaving the tag pages intact.

The per-type mapping, the type-only-on-create rule (and the SDK *hang* it dodges), kebab-casing (except `ISSN`/`ISBN`/`DOI`), per-property visibility (`hide?` / `hide-empty-value`), and the `removeProperty`-not-`deletePage` deletion gotchas are detailed in [`settings.md`](./dev-notes/settings.md) → *Schema application*.

### Web references

The plugin doesn't clip the web — a companion browser extension (separate repo, `logseq-web-clipper`) does, over Logseq's HTTP API. It **reads** the plugin's live `logseq.settings` (under the kebab plugin id) but **cannot write** them, so the setup hub's **Web references** section (`WebSection`) is the only editing surface. The `web*` keys are therefore a **contract**: the extension discovers the schema by walking `webTag`'s *inherited* properties, so the web class must `extends` the base tag (`ensureWebTagExtendsBase`, `services/set-web-schema.ts`) or it aborts the clip ("schema not set up"). **Renaming any `web*` key — or the plugin id — breaks the extension** unless its mapping (`logseq-remote-settings.ts`) / single-sourced `LOGSEQ_PLUGIN_ID` is updated in lockstep.

The complete key contract — every `web*` key, the page-template/section model, and the shared creator-formatting keys the clipper also reads — is in [`settings.md`](./dev-notes/settings.md) → *Page template* + *Author formatting*; full handoff context is the extension repo's `LOGSEQ_SETTINGS_INTEGRATION.md`.
...
**The annotation core** (`src/services/pdf-annot/`) started as a faithful, golden-tested port of the [`pdf-annot-logseq`](../pdf-annot-logseq) prototype's stage 1 — `extract` (native PDF `/Annots` → records), `convert` (→ the Logseq `hl-value` geometry shape), `zotero` (Zotero-DB annotations → the same shape), `geometry`/`colors`/`pdf-pages`/`uuid`/`edn`/`validate`. The PDF engine is **mupdf** (WASM); `extract.ts` / `pdf-pages.ts` are the only modules that touch it. It's now **first-party** — do new annotation work here; the prototype was the lab where the coordinate math was worked out and validated, not an upstream to sync from (the dir is self-contained, with no imports from it). That math is delicate (the transform is validated to ~1e-13), so the **golden tests + fixtures under `__fixtures__/` are the contract**: change the math deliberately and keep `bun test` green — re-blessing fixtures when a change is intentional — rather than casually refactoring it.
...
**Setup:** needs Logseq's **HTTP APIs Server** enabled + its token saved as `logseqApiToken` (set in the hub's **Annotations** section, `AnnotationsSection`). `annotationColor` (`auto` = nearest-pastel, or a forced color) and `logseqApiBaseUrl` live there too. The Annotations section also has an opt-in `annotationColorPerType` toggle that swaps the single color for three per-category swatches (`annotationColorMarkup`/`Text`/`Note` — exhaustive over what we import: markup = Highlight/Underline/StrikeOut/Squiggly, text = FreeText, note = the sticky pin); `import-annotations` then passes `convert`/`convertZoteroAnnotations` a `colorByType` map resolved per record (`categoryForSubtype` + `resolveColor`) instead of a flat `color`.
...
`<% placeholder %>` strings fill `pagenameTemplate` (`<% citeKey %>`, `<% title %>`) and `creatorNameTemplate` (`<% firstName %>`, `<% lastName %>`), resolved by pure, tested functions in `services/resolve-templates.ts` (`applyPageNameTemplate` / `applyCreatorTemplate`). `handle-zot-db.ts` wraps them with the value from settings, and the Formats/Authors previews render through the *same* functions, so preview == import output. The grammar, case/whitespace tolerance, and the collision-safe fallback are detailed in [`settings.md`](./dev-notes/settings.md) → *Templates*. (The `inGraph` badge doesn't use templates — it matches the `zotero-code` property, not a templated name.)
...
- React 19, react-hook-form for the search form, date-fns for dates, wretch for HTTP, lucide-react for icons, `@dnd-kit` for the one drag-reorder list (web sections). Search has **no client-side fuzzy index**: `hooks/use-items.ts` caches the recents snapshot and, while typing, queries Zotero's own SQLite index server-side (`/items/top?q=…&qmode=everything`, debounced) — single-digit ms on localhost. (An earlier local fuse.js pass was removed; don't reintroduce one.)
