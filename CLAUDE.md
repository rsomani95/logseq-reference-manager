...
6. Attachments + annotations get inserted under a wrapping block whose name is the `attachmentsBlockName` setting (default `Attachments`). Per attachment, the emitted block depends on its Zotero `linkMode`:
   ...
   Each per-attachment block carries the `zotero-attachment-key` property so the sync flow can match against it; annotations land as its children, sorted by `annotationSortIndex`.
...
`services/sync-annotations.ts` reads `zotero-code` and `zotero-last-sync` from the page properties, fetches annotations from Zotero added after `zotero-last-sync` (via `getAnnotationsByItemKey` — note: annotations in Zotero are grandchildren, parent → attachment → annotation), finds the wrapping block by exact match against the current `attachmentsBlockName` setting (no legacy-name fallback — see **Backward compatibility**), then finds the right attachment block under it by matching its `zotero-attachment-key`, and appends new annotation blocks. After success, `zotero-last-sync` is updated.
...
Past example: when the attachments wrapper block became user-configurable, `syncAnnotations` was reduced to a single exact-match against `attachmentsBlockName` — no fallback set for the prior `Attachments and Annotations` / `## Attachments and Annotations` strings.
