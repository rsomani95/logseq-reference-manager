...
DB-graph Logseq plugin SDK quirks (property API — including the `hide?` deletion/display gotchas — theming, dev workflow, debugging) and the local HTTP API for probing a running graph are collected in [`LOGSEQ_SDK_NOTES.md`](./dev_notes/LOGSEQ_SDK_NOTES.md). Check there first when an `@logseq/libs` call behaves unexpectedly.

## Architecture

> New to this codebase? Start with [`dev_notes/ARCHITECTURE.md`](./dev_notes/ARCHITECTURE.md) (high-level framing, tech stack, the cross-process mental model) and [`dev_notes/MODULE_MAP.md`](./dev_notes/MODULE_MAP.md) (where everything lives). The sections below are the detailed reference those two point into.
