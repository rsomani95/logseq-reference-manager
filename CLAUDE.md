...
- **Zotero → Import formats** (`FormatsSection`) — Zotero-only page naming + attachment behavior: `pagenameTemplate` / `pagenamePrefix` (dropdown + prefix input) with a live page-name preview (shared `useFmtSample`), plus `openAttachmentInline`. Author formatting moved to **General → Authors**; nothing left here touches the schema, so there's no re-apply footer.
